...
import { getAvatarColor, getComparisonButtonLabelForIndex, formatComparisonValue, getChartValueFilter } from '../modules/UiHelper';
            ...
            <>
                {
                    state.comparisonTickerData.map(item => {
                        ...
                        let slimList = [];
                        const initialValue = item.series[item.series.length - 1].value;
                        const delta = 1;
                        for (i = 0; i < item.series.length; i = i + delta) {
                            slimList.push(item.series[i].value / initialValue * 100);
                        }
